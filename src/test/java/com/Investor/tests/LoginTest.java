...
import com.locators.allPages.ViewAccountInvestorPO;
...
public class LoginTest extends Baseclass {
	...
	@Test(priority=12)
	public void validateViewAccountTextButtonForInvestor() throws IOException {
		try {
			extenttest = extent.createTest("Verify that user is able to view account text  button only for Investor role")
					.assignCategory(investor_tag + "_" + env);
			node = extenttest.createNode("Report");
			
			
			String username = getDataFromExcel(1);
			String password = getDataFromExcel(2);
		...
		// Wait for the user details on the listing page
		 node.log(Status.INFO, "STEP3 : Waiting for the user details on the listing page");
		 System.out.println("STEP3 : Waiting for the user details on the listing page");
		 IWaitStrategy.waitForVisiblity(node, driver, UsersDatabasePO.editIcon);
		 // Click on the username field on the database page.
		   IElementActions.clickAndInput_usingXpath(extenttest, driver, UsersDatabasePO.usernameSearchBox,getDataFromExcel(71) );
		   node.log(Status.INFO, "STEP1 : Username entered by user on the database page");
		   System.out.println("STEP1 : Username entered by user on the database page");
		   ...
		   //Click on the edit icon
			...
			node.log(Status.INFO, "STEP3 : Waiting for the visibility of edit user page");
			System.out.println("STEP3 : Waiting for the visibility of edit user page");
			IWaitStrategy.waitForVisiblity(node, driver, ViewAccountInvestorPO.viewAccountButton);
			node.log(Status.INFO, "STEP4 : Edit user page is visible");
			System.out.println("STEP4 : Edit user page is visible");			
			
		   //Check the view account button for investor user. 
			
			IElementActions.assertTrue_usingXpath(extenttest, driver, ViewAccountInvestorPO.viewAccountButton,
					"STEP5 : BUG:View Account button is not coming for the investor account profile",
					"STEP5 : EXPECTED:View Account button is coming for the investor account profile");
			// Take Screenshot of View button after user navigate on the investor profile page
			IScreenAction.captureScreenShot_PassCase("validateViewAccountTextButtonForInvestor", "Pass", driver);
			
			

		} catch (AssertionError validateViewAccountTextButtonForInvestor) {

			node.log(Status.INFO,
					"EXCEPTION BLOCK 1 : FAIL : Some Assertion Error Caught In validateViewAccountTextButtonForInvestor test in LoginTest class");

			// Take the failed case screenshot
			String screenpath = IScreenAction.captureScreenShot_FailedCase(driver,
					"validateViewAccountTextButtonForInvestor_Fail");
			node.log(Status.FAIL, "Some Assertion Error" + extenttest.addScreenCaptureFromPath(screenpath));

			validateViewAccountTextButtonForInvestor.printStackTrace();
			node.fail(validateViewAccountTextButtonForInvestor);
			Assert.fail();
			extent.flush();
		}

		catch (Exception validateViewAccountTextButtonForInvestor) {
			node.log(Status.INFO,
					"EXCEPTION BLOCK 2 : FAIL : Some Exception Error Caught In validateViewAccountTextButtonForInvestor test in LoginTest class");
		...
		@Test
		public void validateDashboardRedirection() throws IOException {
			try {
				extenttest = extent.createTest("Verify that user is redirected to the Investor Portal Dashbaord on clicking on View Account")
						.assignCategory(investor_tag + "_" + env);
				node = extenttest.createNode("Report");
			...
			// Wait for the user details on the listing page
			 ...
			 IWaitStrategy.waitForVisiblity(node, driver, UsersDatabasePO.editIcon);
			 // Click on the username field on the database page.
			   IElementActions.clickAndInput_usingXpath(extenttest, driver, UsersDatabasePO.usernameSearchBox,getDataFromExcel(75) );
			   node.log(Status.INFO, "STEP1 : Username entered by user on the database page");
			   System.out.println("STEP1 : Username entered by user on the database page");
			   ...
			   //Click view account button and verify that user is navigating to the investor dashboard. 
				
				IElementActions.clickelement_usingXpath(node, driver, ViewAccountInvestorPO.viewAccountButton);
				
				IElementActions.assertTrue_usingXpath(extenttest, driver, ViewAccountInvestorPO.dashboard,
						"STEP5 : BUG:User is not navigate to investor dashboard page",
						"STEP5 : EXPECTED:User is navigate to investor dashboard page");
				// Take Screenshot of View button after user navigate on the investor dashboard page
				IScreenAction.captureScreenShot_PassCase("validateDashboardRedirection", "Pass", driver);
				
				

			} catch (AssertionError validateDashboardRedirection) {
				...
				validateDashboardRedirection.printStackTrace();
				node.fail(validateDashboardRedirection);
				Assert.fail();
				extent.flush();
			}

			catch (Exception validateDashboardRedirection) {
				node.log(Status.INFO,
						"EXCEPTION BLOCK 2 : FAIL : Some Exception Error Caught In validateViewAccountTextButtonForInvestor test in LoginTest class");
		...
		@Test
		public void validateInvestorDashboardViewButton() throws IOException {
			try {
			...
			// Wait for the user details on the listing page
			 ...
			 // Click on the username field on the database page.
			   ...
			   //Click view account button and verify that user is navigating to the investor dashboard. 
			...
			} catch (AssertionError validateInvestorDashboardViewButton) {
				...
				validateInvestorDashboardViewButton.printStackTrace();
				node.fail(validateInvestorDashboardViewButton);
				Assert.fail();
				extent.flush();
			}

			catch (Exception validateInvestorDashboardViewButton) {
				node.log(Status.INFO,
						"EXCEPTION BLOCK 2 : FAIL : Some Exception Error Caught In validateViewAccountTextButtonForInvestor test in LoginTest class");
